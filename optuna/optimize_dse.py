...
def objective_single(trial: optuna.Trial, args, workspace_root: str) -> float:
    ...
    place_density = trial.suggest_float("PLACE_DENSITY", args.min_density, args.max_density)
...
def plot_results(study: optuna.Study, multi_objective: bool, output_file: str):
    ...
    with PdfPages(output_file) as pdf:
        ...
        if feasible:
            ...
            scatter = ax.scatter(
                core_utils, areas, c=powers,
                cmap="plasma", s=250, alpha=0.8,
                edgecolors="black", linewidth=1.5, marker="o", zorder=3
            )
            ...
            if len(core_utils) > 2:
                ...
                ax.plot(x_trend, p(x_trend), "b--", linewidth=2, alpha=0.5,
                       label="Trend (2nd order)", zorder=2)

            # Highlight best area point
            best_idx = areas.index(min(areas))
            ax.scatter(
                [core_utils[best_idx]], [areas[best_idx]],
                c="green", s=500, alpha=1.0, edgecolors="black", linewidth=3,
                marker="*", label="Lowest Area", zorder=5
            )
        ...
        if infeasible:
            ax.scatter(
                ...
                label="Timing Violation", marker="x", zorder=1
            )
        ...
        if failed_trials:
            ...
            ax.scatter(
                ...
                label=f"Build Failed ({len(failed_trials)})", marker="X", zorder=10
            )
        ...
        if feasible:
            ...
            best_idx = areas.index(min(areas))
            ax.scatter(
                [densities[best_idx]], [areas[best_idx]],
                c="green", s=500, alpha=1.0, edgecolors="black", linewidth=3,
                marker="*", label="Lowest Area", zorder=5
            )
        ...
        if multi_objective and feasible:
            ...
            ax.scatter(
                all_areas, all_powers,
                c="lightblue", s=180, alpha=0.5, edgecolors="gray", linewidth=1,
                label=f"Feasible ({len(feasible)} points)", marker="o", zorder=2
            )

            # Highlight Pareto optimal points
            if pareto_areas:
                ax.scatter(
                    pareto_areas, pareto_powers,
                    c="red", s=400, alpha=0.95, edgecolors="black", linewidth=2.5,
                    label=f"Pareto Optimal ({len(pareto_areas)})", marker="*", zorder=10
                )
                ...
                if len(pareto_areas) > 1:
                    ...
                    ax.axvspan(max_area, ax.get_xlim()[1], alpha=0.1, color="red", label="Dominated Region")
            ...
            ax.axvline(all_areas[best_area_idx], color="blue", linestyle=":", alpha=0.5, linewidth=2, label="Best Area")
            ax.axhline(all_powers[best_power_idx], color="green", linestyle=":", alpha=0.5, linewidth=2, label="Best Power")

            ax.set_xlabel("Cell Area (um²)", fontsize=14, fontweight="bold")
            ax.set_ylabel("Power (uW)", fontsize=14, fontweight="bold")
            ax.set_title("Area-Power Pareto Frontier\n(Lower-left is better)", fontsize=16, fontweight="bold", pad=15)
        ...
        if trial_numbers:
            ...
            scatter = ax.scatter(
                trial_numbers, areas, c=powers, cmap="coolwarm",
                s=200, alpha=0.7, edgecolors="black", linewidth=1.5,
                label="Trial Results", zorder=3
            )
            ...
            ax.plot(trial_numbers, best_so_far, "g-", linewidth=3.5,
                   label="Best So Far", zorder=4, alpha=0.8)
            ...
            if improvement_trials:
                ...
                ax.scatter(
                    imp_nums, imp_areas,
                    c="gold", s=400, alpha=1.0, edgecolors="black", linewidth=2.5,
                    marker="D", label="Improvement", zorder=5
                )
                ...
                ax.annotate(
                    ...
                    bbox=dict(boxstyle="round,pad=0.7", fc="gold", alpha=0.9, ec="black", lw=2),
                    arrowprops=dict(arrowstyle="->", connectionstyle="arc3,rad=0.3", lw=2.5),
            ...
            if len(areas) > 1:
                ...
                ax.text(
                    ...
                    bbox=dict(boxstyle="round,pad=0.8", fc="lightblue", alpha=0.8, ec="black", lw=1.5)
                )

        ax.set_xlabel("Trial Number", fontsize=14, fontweight="bold")
        ax.set_ylabel("Cell Area (um²)", fontsize=14, fontweight="bold")
        ax.set_title("Optimization Convergence History", fontsize=16, fontweight="bold", pad=15)
        ...
        if len(feasible) > 3:
            ...
            for i, (cu, pd, area) in enumerate(zip(core_utils, densities, areas)):
                ax.annotate(
                    f"{area:.2f}",
                    xy=(cu, pd), xytext=(0, 0), textcoords="offset points",
            ...
            best_idx = areas.index(min(areas))
            ax.scatter(
                [core_utils[best_idx]], [densities[best_idx]],
                s=700, facecolors="none", edgecolors="blue", linewidth=4,
            ...
            if infeasible:
                ax.scatter(
                    [t["core_util"] for t in infeasible],
                    [t["place_density"] for t in infeasible],
                    c="gray", s=200, alpha=0.4, edgecolors="red", linewidth=2,
                    marker="x", label="Timing Violation", zorder=2
                )
            ...
            ax.text(
                ...
                bbox=dict(boxstyle="round,pad=0.8", fc="lightyellow", alpha=0.95, ec="blue", lw=2.5)
            )

            # Add secondary scatter with power as size
            scatter2 = ax.scatter(
                core_utils, densities, s=[p*15 for p in powers],
                facecolors="none", edgecolors="purple", linewidth=1.5,
                alpha=0.4, label="Size ∝ Power", zorder=2
            )
...
def main():
    ...
    parser.add_argument(
        "--min-util", type=int, default=30,
        help="Minimum CORE_UTILIZATION %% (default: 30)"
    )
    parser.add_argument(
        "--max-util", type=int, default=70,
        help="Maximum CORE_UTILIZATION %% (default: 70)"
    )
    parser.add_argument(
        "--min-density", type=float, default=0.20,
        help="Minimum PLACE_DENSITY (default: 0.20)"
    )
    parser.add_argument(
        "--max-density", type=float, default=0.70,
    ...
    parser.add_argument(
        "--output-dir", type=str, default="optuna/results",
    ...
    print(f"Mode: {'Multi-objective (Area+Power)' if args.multi_objective else 'Single-objective (Area)'}")
    print("=" * 70)
    ...
    if args.multi_objective:
        study = optuna.create_study(
            ...
            sampler=optuna.samplers.TPESampler(seed=args.seed, constraints_func=constraints)
        )
        study.optimize(
