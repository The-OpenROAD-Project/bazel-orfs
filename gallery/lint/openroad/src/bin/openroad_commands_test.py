...
class TestSDC:
    def test_read_sdc_missing_file(self, interp, capsys):
        interp.eval(
            "read_sdc /nonexistent/constraints.sdc"
        )
    ...
    def test_read_sdc_existing(self, interp, state):
        with tempfile.NamedTemporaryFile(
            mode="w", suffix=".sdc", delete=False
        ) as f:
...
_LINT_VARS = [
    "PLATFORM", "DESIGN_NAME", "CORE_UTILIZATION",
    "CORE_ASPECT_RATIO", "CORE_MARGIN", "CORE_AREA",
    "DIE_AREA", "MOCK_AREA", "PLACE_DENSITY",
    "ROUTING_LAYER_ADJUSTMENT", "TNS_END_PERCENT",
    "RECOVER_POWER", "MACRO_PLACE_HALO",
    "CELL_PAD_IN_SITES_GLOBAL_PLACEMENT",
    "CELL_PAD_IN_SITES_DETAIL_PLACEMENT",
    "DETAILED_ROUTE_END_ITERATION",
    "MIN_PLACE_STEP_COEF", "MAX_PLACE_STEP_COEF",
    "SKIP_INCREMENTAL_REPAIR", "SKIP_CTS_REPAIR_TIMING",
    "SKIP_LAST_GASP", "GPL_TIMING_DRIVEN",
    "SYNTH_HIERARCHICAL", "SYNTH_GUT",
...
class TestFloorplan:
    ...
    def test_invalid_utilization_zero(
        self, interp, capsys
    ):
    ...
    def test_invalid_utilization_over_100(
        self, interp, capsys
    ):
    ...
    def test_die_area_sky130_large_ok(
        self, interp, capsys
    ):
...
class TestLintNumericRanges:
    ...
    def test_routing_layer_adj_valid(
        self, interp, capsys
    ):
    ...
    def test_routing_layer_adj_over_1(
        self, interp, capsys
    ):
    ...
    def test_tns_end_percent_valid(
        self, interp, capsys
    ):
    ...
    def test_tns_end_percent_negative(
        self, interp, capsys
    ):
    ...
    def test_recover_power_over_100(
        self, interp, capsys
    ):
    ...
    def test_core_aspect_ratio_valid(
        self, interp, capsys
    ):
    ...
    def test_core_aspect_ratio_extreme(
        self, interp, capsys
    ):
    ...
    def test_core_aspect_ratio_zero(
        self, interp, capsys
    ):
    ...
    def test_min_place_step_coef_valid(
        self, interp, capsys
    ):
    ...
    def test_min_place_step_coef_extreme(
        self, interp, capsys
    ):
...
class TestLintCrossVariable:
    def test_util_and_die_area_both_set(
        self, interp, capsys
    ):
        os.environ["CORE_UTILIZATION"] = "40"
        os.environ["DIE_AREA"] = "0 0 50 50"
    ...
    def test_core_area_without_die(
        self, interp, capsys
    ):
    ...
    def test_macro_halo_wrong_count(
        self, interp, capsys
    ):
...
class TestLef:
    def test_read_lef_missing(self, interp, capsys):
        interp.eval(
            "read_lef /nonexistent/tech.lef"
        )
        captured = capsys.readouterr()
        assert "not found" in captured.err

    def test_write_abstract_lef_insane_size(
    ...
    def test_write_timing_model(
        self, interp, state, tmpdir
    ):
        state.design_name = "test_macro"
        state.cell_count = 500
        path = os.path.join(
            tmpdir, "test_macro_typ.lib"
        )
...
class TestMockODB:
    ...
    def test_method_chain(self, interp):
        ...
        result = interp.eval(
            "[$db getTech] getDbUnitsPerMicron"
        )
        assert int(result) > 0
...
class TestSDCValidation:
    ...
    def test_get_ports_unknown_warns(
        self, interp, state, capsys
    ):
        state.ports = {"clk", "rst"}
    ...
    def test_get_ports_multi_arg_warns(
        self, interp, state, capsys
    ):
        state.ports = {"clk"}
    ...
    def test_get_ports_quiet_no_multi_warn(
        self, interp, state, capsys
    ):
        state.ports = {"clk"}
    ...
    def test_get_ports_wildcard_no_warn(
        self, interp, state, capsys
    ):
        state.ports = {"clk"}
    ...
    def test_create_clock_unknown_warns(
        self, interp, state, capsys
    ):
        state.ports = {"clk"}
        interp.eval(
            "create_clock -period 1000 bad_port"
        )
        captured = capsys.readouterr()
        assert "not in design" in captured.err

    def test_group_path_empty_from_ok(
        self, interp, capsys
    ):
    ...
    def test_set_input_delay_unknown_warns(
        self, interp, state, capsys
    ):
        state.ports = {"clk", "data_in"}
        interp.eval(
            "set_input_delay -clock clk 0 bad_port"
        )
        captured = capsys.readouterr()
        assert "not in design" in captured.err

    def test_set_output_delay_unknown_warns(
        self, interp, state, capsys
    ):
        state.ports = {"clk", "data_out"}
        interp.eval(
            "set_output_delay -clock clk 0 bad_port"
        )
        captured = capsys.readouterr()
        assert "not in design" in captured.err
...
class TestAllRegistersInputsOutputs:
    ...
    def test_all_inputs_returns_ports(
        self, interp, state
    ):
...
class TestORFSHelpers:
    def test_source_env_var_if_exists_missing(
        self, interp
    ):
        """source_env_var_if_exists with unset var is no-op."""
        os.environ.pop("NONEXISTENT_VAR", None)
        interp.eval(
            "source_env_var_if_exists NONEXISTENT_VAR"
        )

    def test_env_var_exists_and_non_empty(
        self, interp
    ):
        os.environ["TEST_VAR"] = "hello"
        result = interp.eval(
            "env_var_exists_and_non_empty TEST_VAR"
        )
        assert result == "1"
...
class TestReadVerilogPorts:
    ...
    def test_no_ports_from_missing_file(
        self, interp, state
    ):
