...
and startpoint pins; an older four-column dump is read as is). The slack
used is the STA's: a per-path repricing cannot reorder the worst-N
selection, which the STA made over the whole graph, so the fanout model
belongs in the netlist (repair_design -pre_placement) and the priced
column is a diagnostic of how much of a path's violation is unbuffered
fanout. Each
...
def read_paths(path, mods):
    ...
    for line in open(path):
        ...
        if len(p) < 4:
            continue
        if len(p) >= 6:
            # six-column dump: slack, priced slack, pins, max fanout, end,
            # start. The table's number is the STA's slack: the pricing is
            # a per-path diagnostic, and the dump's worst-N selection is
            # the STA's, so the netlist has to carry the fanout model
            # (repair_design -pre_placement) before the slacks mean much
            slack, pins, end, start = float(p[0]), int(p[2]), p[4], p[5]
        else:
