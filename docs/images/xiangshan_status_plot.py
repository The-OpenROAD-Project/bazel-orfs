#!/usr/bin/env python3
"""Render the take 23 status figures, one PNG per panel, from the two JSON files beside it.
...
def panel_record(ax):
    ax.set_title(
        "The record: large designs in the flow repositories",
        loc="left",
        fontsize=11,
        weight="bold",
    )
    ...
    ax.text(
        0.15,
        0.1,
        "XSCore: %.1f M cells after synthesis, %.2f M placed with the arrays"
        % (4.3, (D["cells_parent"] + D["cells_arrays_firm"]) / 1e6),
        fontsize=8,
        color="0.3",
...
def panel_sits(ax):
    ax.set_title(
        "Where XSCore sits in the repository's own SoC (areas roughly to scale)",
        loc="left",
        fontsize=11,
        weight="bold",
    )
    ...
    ax.text(
        0.3,
        outer_h - 0.7,
        "NoC top: two XSTiles on the CHI network of chip + one shared LLC (the shipped topology supports one or two cores)",
        fontsize=8,
    )
    x = 0.4
    for t in range(2):
    ...
    ax.text(
        x + llc / 2,
        2.5,
        "OpenLLC, shared last-level cache, 32 MB default",
        ha="center",
        fontsize=8,
    )
    ax.text(
        0.3,
        -0.9,
        "red = the block this study places: the core with its L1 caches and TLBs, %.1f M cells; no L2, no NoC, no LLC. This study's L2 is 512 KB and its LLC 1 MB."
        % 4.3,
        fontsize=8,
    )
...
def panel_ladder(ax):
    ax.set_title(
        "Minimum clock period measured vs the public target (log scale)",
        loc="left",
        fontsize=11,
        weight="bold",
    )
    ...
    bars = ax.barh(
        range(len(rows)), vals, color=[colors[k] for k in kinds], edgecolor="k"
    )
    ...
    for i, v in enumerate(vals):
        ax.text(
            v * 1.08,
            i,
            "%d ps  (%.0fx)" % (v, v / vals[0]) if i else "%d ps" % v,
            va="center",
            fontsize=8,
        )
    ...
    ax.text(
        0.02,
        0.02,
        "hatched: a model artefact, the abstracts were written before the blocks' own CTS (inventory entry 17)",
        fontsize=7,
        color="0.3",
...
def panel_die(ax):
    ax.set_title(
        "The die as run: 3.6 x 2.1 mm, four blocks, three arrays, route-0 overflow at M9",
        loc="left",
        fontsize=11,
        weight="bold",
    )
    ...
    ax.imshow(
        g,
        origin="lower",
        extent=(0, HEAT["nx"] * HEAT["bin_um"], 0, HEAT["ny"] * HEAT["bin_um"]),
        cmap="Reds",
        alpha=0.9,
        aspect="equal",
    )
    for name, (x, y, w, h) in D["blocks"].items():
        ...
        ax.text(
            x + w / 2, y + h / 2, name, ha="center", va="center", color="w", fontsize=8
        )
    ...
    ax.text(
        (x0 + x1) / 2, -110, "%d top-level ports" % D["ports"], ha="center", fontsize=7
    )
    ax.set_xlim(0, W)
    ax.set_ylim(-150, H)
    ax.set_xlabel("um")
    ...
    ax.text(
        W - 30,
        1750,
        "route-0 (0 iterations): %.0f%% of capacity used,\nworst edge %d wires over, total congestion %.0f M;\nfive-iteration route killed at 3 h"
        % (r["usage_pct"], r["max_h"], r["total"] / 1e6),
        fontsize=7,
        ha="right",
        bbox=dict(fc="w", ec="0.5"),
    )


def panel_hours(ax):
    ax.set_title(
        "Where the hours go (one iteration; peak memory in GB)",
        loc="left",
        fontsize=11,
        weight="bold",
    )
    ...
    for i, (h, m) in enumerate(zip(hours, mem)):
        label = (
            "%.1f h" % h
            + (", %.0f GB" % m if m else "")
            + ("  killed at budget" if "global route" in names[i] else "")
        )
    ...
    ax.set_xlim(0, 4.5)
    ax.set_xlabel(
        "hours   (total %.1f h to the route's kill; %.2f M cells + %.2f M fixed array cells; %d block pins)"
        % (
            sum(hours),
            D["cells_parent"] / 1e6,
            D["cells_arrays_firm"] / 1e6,
            D["block_pins"],
        ),
        fontsize=8,
    )
...
def panel_extracted(ax):
    ax.set_title(
        "6. From gold mine to test case: what the campaign found, and the minute-scale test it became",
        loc="left",
        fontsize=11,
        weight="bold",
    )
    ...
    ax.text(
        0.1,
        len(rows) + 0.5,
        "found on the design (hours to days)",
        fontsize=8,
        weight="bold",
    )
    ax.text(
    ...
    for i, (found, became, kind) in enumerate(rows):
        ...
        ax.annotate(
            "", xy=(5.2, y), xytext=(4.7, y), arrowprops=dict(arrowstyle="->", color=c)
        )
        ax.text(5.3, y, became, fontsize=7.5, va="center", color=c)
    ax.text(
        0.1,
        0.2,
        "green: a test in the repository   blue: a carried ORFS patch   red: not yet extracted, an open OpenROAD question",
        fontsize=7,
        color="0.3",
    )
...
PANELS = [
    ("xiangshan-take23-record.png", panel_record, (11, 4.2)),
    ("xiangshan-take23-soc.png", panel_sits, (12, 4.5)),
    ("xiangshan-take23-period-ladder.png", panel_ladder, (11, 4.5)),
    ("xiangshan-take23-die.png", panel_die, (12, 8)),
    ("xiangshan-take23-hours.png", panel_hours, (10, 4.5)),
]


def main():
    """One PNG per panel; the finding-to-test table (panel 6) is prose in the report."""
