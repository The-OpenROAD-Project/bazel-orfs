# Shared KPI extraction, factored out of extract.tcl so the flow-side
# variance walk (stage_variance.tcl) measures its leaves with the same
# instrument that measures the ground truth.  A study that compares
# spreads across ensembles is only as good as the identity of its
# measurement, so there is exactly one copy of the sampling logic.
