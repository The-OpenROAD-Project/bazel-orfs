...
class PlanDriftTest(unittest.TestCase):
    def test_plan_dir_matches_the_generators(self):
        ...
        subprocess.check_call(
            [sys.executable, os.path.join(here, "mini_gen.py"), "--out", out]
        )
        ...
        subprocess.check_call(
            [
                planner,
                os.path.join(out, "plan.json"),
                "--out",
                os.path.join(out, "plan_out.json"),
                "--emit",
                out,
            ],
            cwd=root,
        ...
        for f in [
            ...
            with open(os.path.join(out, f)) as a, open(
                os.path.join(here, "plan", f)
            ) as b:
                self.assertEqual(
                    a.read(),
                    b.read(),
                    "%s drifted: run test/planned_parent/regen.sh" % f,
                )
