...
class CommandTest(unittest.TestCase):
    def test_make_command(self):
        ...
        arm = {
            "args": "-congestion_iterations 5",
            "layers": "M2 M5",
            "openroad": "/opt/or/openroad",
            "pin_access": False,
        }
        argv = grt_bench.make_command(
            design, arm, "/r/xs__a__r0.json", {"args": "-allow_congestion"}
        )
        ...
        self.assertTrue(
            any(
                a.startswith("ODB_FILE=")
                and a.endswith("/_main/results/asap7/x/base/4_cts.odb")
                for a in argv
            )
        )
    ...
    def test_scope_command_caps(self):
        cmd = grt_bench.scope_command(
            ["make", "run"], "u", {"memory_max": "60G", "swap_max": "70G"}
        )
