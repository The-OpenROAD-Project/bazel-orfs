# ibex with its instruction cache on, built to global route only.
#
# Not the point this study reports. Section 5.1 has the reasoning: a
# cache behind a single-cycle tightly-coupled memory cannot buy a cycle
# -- CoreMark/MHz is identical with it on and off, to the digit -- and
# ibex's 4 kB cannot hold CoreMark's 24-30 kB of .text, so it cannot be
# exercised at this study's boundary either. What it does change is
# energy, and mostly for a reason that belongs to the memory model
# rather than to ibex.
#
# This design exists so that none of that is asserted from memory. It
# keeps the ASAP7 prim_ram_1p and the two cache-RAM macros built, so a
# configuration that silently reverted to flip-flops -- 44,032 of them
# -- would fail here rather than in a number nobody re-derives.
export PLATFORM                = asap7

export DESIGN_NAME             = cmj_ibex_ic
export DESIGN_NICKNAME         = ibex_icache
...
export VERILOG_FILES           = @ibex//:pkgs @ibex//:rtl_tech_ram \
                                 //test/coremark_joule/rtl:cmj_prim_ram_1p.sv \
                                                                  //test/coremark_joule/rtl:cmj_ibex.sv \
                                 //test/coremark_joule/rtl:cmj_ibex_ic.sv \
                                 //test/coremark_joule/rtl:cmj_progmem.sv //test/coremark_joule/flow:cmj_sram_blackbox.v \
                                 //test/coremark_joule/rtl:cmj_dmem.v
export SDC_FILE                = $(DESIGN_HOME)/asap7/ibex_icache/constraints.sdc
...
# No ibex_prefetch_buffer or ibex_fetch_fifo: ibex_icache replaces the
# pair when ICache=1, and synth.tcl errors on a kept name the
# elaborated design does not have -- which is how this list is kept
# honest against a configuration change.
export SYNTH_KEEP_MODULES      = ibex_if_stage ibex_icache \
...
export ADDITIONAL_LEFS         = //test/coremark_joule/flow:cmj_imem_sram.lef \
                                 ...
                                 //test/coremark_joule/flow:cmj_ic_tag_sram.lef \
                                 //test/coremark_joule/flow:cmj_ic_data_sram.lef
export ADDITIONAL_LIBS         = //test/coremark_joule/flow:cmj_imem_sram.lib \
                                 //test/coremark_joule/flow:cmj_dmem_lane_sram.lib \
                                 //test/coremark_joule/flow:cmj_ic_tag_sram.lib \
                                 //test/coremark_joule/flow:cmj_ic_data_sram.lib
