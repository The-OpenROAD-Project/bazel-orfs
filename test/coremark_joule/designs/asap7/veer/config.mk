...
# separates the two cases: it guards rvdffe's generate block only, and
# beh_lib.sv defines the gate either way. What differs is the name --
# ORFS's vendored copy points TEC_RV_ICG at a TSMC cell, this one at
# `clockhdr` -- and, because beh_lib.sv defines whatever name the macro
# carries, the module has to be substituted after elaboration rather
# than redirected before it.
...
# see units.json.
# Map VeeR's clock gate onto ASAP7's ICG cell.
#
# SYNTH_POST_HIERARCHY_SCRIPTS (patches/0068) runs a yosys script while
# the hierarchy is intact, which is the only point at which a clockhdr
# instance is still a cell of its own type; synth's own -extra-map comes
# after flatten. cmj_veer_icg.ys is the script and carries the reasoning.
#
# Named by sandbox path, as VERILOG_INCLUDE_DIRS below is and for the
# same reason: this is an argument variable, so a label in it is neither
# staged nor rewritten. VEER_ICG_MAP is what stages the two files.
export SYNTH_POST_HIERARCHY_SCRIPTS = test/coremark_joule/flow/cmj_veer_icg.ys

# Staged into the sandbox for the hook above, never handed to the
# frontend: one is a yosys script, the other a techmap template, and
# reading the template as a source would define a second clockhdr
# alongside beh_lib.sv's.
export VEER_ICG_MAP            = //test/coremark_joule/flow:cmj_veer_icg.ys \
                                 //test/coremark_joule/flow:cmj_veer_icg_map.v
