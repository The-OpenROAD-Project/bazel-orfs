...
the same sweep moves the vectorless total by 23--179 %. The energy
...
|---|---|---|---|---|---|---|
| SERV | rv32i | 0.0243 | 41,202,900 | 1428.6 | 28.10 mW | 1,234 |
| picorv32 | rv32im | 0.5531 | 1,807,889 | 1000.0 | 21.40 mW | 25,847 |
| ibex | rv32imc | 2.4543 | 407,448 | 833.3 | 20.60 mW | 99,284 |
| VeeR EH1 | rv32imc | 4.7979 | 208,431 | 625.0 | 87.30 mW | 34,349 |
...
|---|---|---|
| 833 MHz (measured) | 10.1 µJ | 99,284 |
| 3.0 GHz (projected) | 131 µJ | 7,660 |
| 5.0 GHz (projected) | 363 µJ | 2,760 |
...
|---|---|---|---|---|---|
| SERV | 27,597 | 27,597 (100.0000 %) | 0 | 0 | pass |
| picorv32 | 52,280 | 52,280 (100.0000 %) | 0 | 0 | pass |
| ibex | 82,396 | 82,396 (100.0000 %) | 0 | 0 | pass |
...
|---|---|---|---|---|
| SERV | **0.0000 %** | 22.82 % | 29.73 mW | 28.059 mW |
| picorv32 | **0.0000 %** | 83.86 % | 31.80 mW | 21.383 mW |
| ibex | **0.0000 %** | 179.10 % | 39.65 mW | 20.630 mW |
...
vectorless total over the same sweep runs 26.81 → 32.93 mW (SERV),
20.45 → 37.60 mW (picorv32), 17.12 → 47.78 mW (ibex) and 78.54 →
213.64 mW (VeeR).

The control arm's spread is the part of this table that is not a
constant of the study, and it orders itself by how much of each design
the estimator is free to invent: 23 % on SERV, 84 % on picorv32, 179 %
on ibex, 172 % on VeeR. The estimator has least room where a macro's
internal power dominates a total it cannot move by seeding an input
activity, and most where the design is logic whose activity it has to
...
|---|---|---|
| extrapolation to VeeR overpredicts by | 15.2x | **5.65x** |
| slope, three cacheless cores | 0.868 | **0.954** |
| R^2 of that fit | 0.999 | **0.9997** |
| `f/P` spread, those three | 1.89x | **1.26x** |
| power spread, those three | 1.15x | 1.36x |
| slope, all four | 0.535 | 0.742 |
| R^2, all four | 0.561 | 0.858 |

**The prediction held, and it was most of the error.** The
extrapolation gap closed from 15.2x to 5.65x: **63 % of the way**. The
...
256x256 (§5.1). Those macros are 58--71 % of each point's power (§4.7).
...
performance per clock at **0.35x** its CoreMark/Joule, drawing 4.24x
...
**A corroboration, still carefully.** VeeR at 34,349 CoreMark/Joule
...
|---|---|---|---|---|---|
| SERV | **28.10 mW** | 4.10 (14.6 %) | 2.86 (10.2 %) | 1.24 (4.4 %) | **19.90 (70.8 %)** |
| picorv32 | **21.40 mW** | 3.54 (16.5 %) | 2.90 (13.6 %) | 0.84 (3.9 %) | **14.10 (65.9 %)** |
| ibex | **20.60 mW** | 3.30 (16.0 %) | 2.65 (12.9 %) | 2.80 (13.6 %) | **11.90 (57.8 %)** |
...
**The memory is the measurement.** 58--71 % of every point: 70.8 % of SERV down to
57.8 % of VeeR. The old rows show what that meant: for these three cores
...
system, a host built from this study's ibex point — 99,284
CoreMark/Joule — would deliver `87.5 × 99,284 ≈ 8.7 M CoreMark/s`, which
...
|---|---|---|---|---|
| SERV | 0.0243 | 5,190 | **1,234** | 4.21x |
| picorv32 | 0.5531 | 86,024 | **25,847** | 3.33x |
| ibex | 2.4543 | 277,510 | **99,284** | 2.80x |
...
The study's cores have no reference point in shipping silicon. This
appendix supplies one, from a deliberately crude measurement, and spends
most of its length on the question that matters: how much of it can be
believed. The short answer is that the **throttling behaviour publishes
and the energy comparison does not** — which is why none of these points
appear on Figure 1.
...
| --- | --- | --- | --- | --- | --- |
| AMD Ryzen Threadripper 3970X | Zen 2 | TSMC N7 [14] | 32 / 64 | 3.9 GHz | 167.0 W |
| Intel Xeon Platinum 8558U | Emerald Rapids | Intel 7 [15] | 48 / 96 | 2.9 GHz | 70.0 W |
| Qualcomm Snapdragon X Elite X1E78100 | Oryon | TSMC N4P [16] | 12 / 12 | 3.417 GHz | 9.2 W |
...
(measured: 52.5 W at the wall for 12 cores) [17] — independent, and it
...
CoreMark/MHz [18]; these parts read 7.38 to 12.53. That database is
...
efficient than N5 [16]; N5 is roughly 30 % over N7), and Oryon is five
...
rescue it, so this is not an artefact of the idle convention; it is
either a real and interesting result about where the energy goes in a
laptop SoC, or the measurement is not resolving what it claims to. **On
this evidence we cannot tell which**, and that is the finding.

### A.5 Verdict, and why these points are not on Figure 1

The throttling half is **viable**: three parts, three distinguishable
and independently corroborated behaviours, from a measurement anyone can
repeat with a plug meter.

The energy half is **not yet publishable as a comparison**. Figure 1
plots points whose boundary is verified by counting every transfer that
leaves the hardened block; putting a wall-plug delta beside them would
assert a comparability that §A.3 and §A.4 have just spent two sections
denying. The CoreMark/MHz axis is boundary-independent and would have
been admissible on its own — but the shared, vendor-specific build
configuration disqualifies it separately. **So Figure 1 is unchanged,
and this appendix is a recipe for the repeat rather than a result to
plot against.**
...
2. **Report the CoreMark validation output and run configuration**, so
   the numbers can be placed against EEMBC's database [18] instead of
   floating above it.
...
14. AMD / WikiChip. *Ryzen Threadripper 3970X* — Zen 2 CCDs on TSMC N7, IO die on GlobalFoundries 12/14 nm. https://en.wikichip.org/wiki/amd/ryzen_threadripper/3970x
15. Intel. *Xeon Platinum 8558U (Emerald Rapids, 5th Gen Xeon Scalable)* — Raptor Cove cores on Intel 7.
16. TSMC. *N4P Extends the Performance and Power Efficiency of the 5nm Family* — N4P is 22 % more power efficient than N5. https://pr.tsmc.com/english/news/2874
17. Independent laptop reviews of the Snapdragon X Elite reporting ~3.4 GHz sustained all-core clocks and ~47.6 W package power under a CPU benchmark.
18. EEMBC. *CoreMark scores database.* https://www.eembc.org/coremark/scores.php
