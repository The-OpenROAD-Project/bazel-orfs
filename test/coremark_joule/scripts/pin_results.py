...
def two_sigma(values):
    ...
    §5.11: the spread is reported as 2σ, and the resolvable difference at
...
def attach_samples(points, sample_paths):
    ...
    (§5.11). The pinned point stays the design's own draw -- seed 1, the
...
def main(argv):
    ...
    parser.add_argument(
        ...
        "point as one sample of its placement-seed ensemble (§5.11)",
    )
    parser.add_argument(
        "--pending",
        action="append",
