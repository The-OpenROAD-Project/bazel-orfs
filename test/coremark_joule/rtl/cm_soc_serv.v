 ...
 * its stack -- lives in the two memories inside cmj_serv, which is what
...
module cm_soc #(
	...
	/* SERV has no trap output: a fault is reported nowhere the
	 * wrapper can see. A broken run therefore shows up as a CRC
	 * mismatch or as the cycle budget expiring, never as a trap, which
	 * is a real difference in failure mode from picorv32 and is why the
	 * harness treats all three exits distinctly. */
	output wire        trap,
