...
def main():
    ...
    ap.add_argument(
        "--results",
        required=True,
        help="directory of campaign results, relative to the workspace",
    )
    ap.add_argument(
        "--policy-clock-ps",
        type=int,
        default=0,
        help="the clock the policy arms ran at, for the section heading",
    )
    ap.add_argument(
        "--figure-base-url",
    ...
    # Discovered rather than declared. The results are not checked in --
    # they are the output of a campaign that costs about forty minutes --
    # so the generator globs whatever a re-run has produced and every
    # section says so when its inputs are absent.
    ...
    if not results.is_dir():
        raise SystemExit(
            "no results directory at %s: run the campaign first, then "
            "the :*_update targets" % results
        )

    def group(prefix, pattern):
        """`<prefix><key>_<stage>.json` -> {key: {stage: data}}."""
        out = {}
        for path in sorted(results.glob(prefix + "*.json")):
    ...
    inv = load(results / "inventory.json")
    if inv is None:
        raise SystemExit("no inventory.json in %s" % results)

    sizes = {
        path.stem.replace("size_g", ""): load(path)
        for path in sorted(results.glob("size_g*.json"))
    }
    shapes = {
        path.stem.replace("shape_", ""): load(path)
        for path in sorted(results.glob("shape_*.json"))
        if not path.name.startswith(("shape_ladder_", "shape_layers_"))
    }
    shape_layers = {
        path.stem.replace("shape_layers_", ""): load(path)
        for path in sorted(results.glob("shape_layers_*.json"))
    }
    shape_ladder = group(
        "shape_ladder_", r"^shape_ladder_(.+)_(?:floorplan|place|cts|grt)\.json$"
    )
    seeds = group("seed_ladder_", r"^seed_ladder_(\d+)_(?:place|grt)\.json$")
    rc_arms = group(
        "rc_ladder_", r"^rc_ladder_(M\d+)_(?:floorplan|place|cts|grt)\.json$"
    )

    policy_period, policy_cost = {}, {}
    for path in sorted(results.glob("policy_*.json")):
        m = re.match(r"policy_(cost_)?t(\d+)_s(\d+)$", path.stem)
        if not m:
            continue
        data = load(path)
        if not data:
    ...
    body = "\n\n".join(
        ...
        for section in [
            ...
            section_rc_transfer(load(results / "rc_fit.json")),
